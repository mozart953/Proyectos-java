...
public class Figura {
     ...
     * valores intervienen en posicion y anchura. <code>Solo aplica en el degradado
     * lineal
     */
    Dupla[] Franja = new Dupla[2];
    /**
     * Es un arreglo de 2 Duplas que representan vectores que modificaran al
     * arreglo de Duplas engadas de graficar el degradado de la forma, sus
     * valores desplazan o modifican la posicion y anchura. <code>Solo aplica en el
     * degradado lineal
     */
    ...
    /**
     * Determina si el degradado se movera o desplazara.
     * <p>
     * <code> si es true se pintara un degradado radial</code>
     * <p>
     * <code> si es false se pintara un degradado lineal</code>.
     */
    ...
    /**
     * Este metodo permite modificar la anchura y angulo del degradado de forma
    ...
    /**
     * Este metodo permite cambiar velocidad, anchura y angulo del degradado de
     * forma aleatoria, con valores entre 0 y 1.
     * <p>
     * para cambiar el angulo y anchura del degradado puedes usar a
     * nuevasFranjasAleatorias()
     *
    ...
    /**
     * Este metodo permite asignar una configuracion de anchura y angulo al
     * degradado.
     * <p>
     * confieso que no se exactamente como funciona, al principio pense que se
     * trataba un par de puntos (x1,y1)(x2,y2) que manipulaban esto, pero luego
     * de hacer algunos experimentos no pude entender bien como funcionaba lo de
     * direccion y anchura
     *
     * @param A
     * @param B
    ...
    /**
     * Este metodo determina si se van a pintar dos colores o uno solo, cuando
    ...
    /**
     * Este metodo detiene o pone en movimiento el deplazamiento de los 2
     * colores.
     * <p>
    ...
    /**
     * Este metodo mueve de forma suave la direccion y anchura del degradado.
     */
    ...
    /**
     * Este metodo controla que las franjas no se pasen de un numero muy alto,
     * por que de ser asi el degradado se expanderia tanto que se volveria asi
     * como un color plano.
     * <p>
     * con lo de franjas me refiero a las lineas de colores que van cambiando
     * respecto al tiempo.
     */
    private void controlador_LimitesDegradado() {
        for (int i = 0; i < Franja.length; i++) {
            ...
            DifFranja[i].X = (Franja[i].X + DifFranja[i].X > maxDegradadoLineal//Determina que no se salga del valor maximo
                    || Franja[i].X + DifFranja[i].X < minDegradadoLineal) ? //Determina que no se salga del valor minimo
                            -DifFranja[i].X : //Lo cambia de sentido
                            DifFranja[i].X; //No le hace nada

            DifFranja[i].Y = (Franja[i].Y + DifFranja[i].Y > maxDegradadoLineal//Determina que no se salga del valor maximo
                    || Franja[i].Y + DifFranja[i].Y < minDegradadoLineal) ? //Determina que no se salga del valor minimo
    ...
    /**
     * Este metodo calcula el degradado en el instante de ejecucion.
     * <p>
     ...
     * <p>
     * es una herramienta auxiliar que uso para acceder de forma rapida a esta
     * acción
     */
    ...
    /**
     * Devuelve un valor aleatorio entre 0 y un valor que se pase por parametro.
     * <p>
     * es una herramienta auxiliar que uso para acceder de forma rapida a esta
     * acción
     *
     * @param n rango Maximo del aleatorio
     */
     ...
     * private a public, hay comentarios que logue dejar porque pensé que este
     ...
     */
    /**
     * Es un punto respecto a la posicion de la figura que determina en que
     * posicion irá el color principal para desvanecerse suavemente hasta el
    ...
    /**
     * Es un punto respecto a la posicion de la figura que determina valor
     ...
     * iteración del hilo a travez del limite radial. <code>solo aplica en el degradado radial
     */
    ...
    /**
     * Es un vector que da la imprecion de inflar o desinflar el degradado
    ...
    private void modificar_MinimoRadial(double v) {
        if (v >= MaximaInflacion) {
            throw new Error("El valor mínimo del radio debe ser menor al maximo valor");
        } else {
            if (v <= 1) {
    ...
    private void modificar_MaximoRadial(double v) {
        if (v <= MinimaInflacion) {
            throw new Error("El valor maximo del radio debe ser mayor al minimo");
        } else {
            MaximaInflacion = v;
        }
